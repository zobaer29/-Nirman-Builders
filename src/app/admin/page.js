    ...
    <div className="w-full space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          ...
          { label: "Website Messages", value: dashboardData?.stats?.unreadContactMessages || "00", icon: "mail", color: "bg-rose-500", isError: true },
        ].map((stat, i) => (
      ...
      <div className="bg-white rounded-[3rem] p-10 border border-slate-100 shadow-sm">
        <div className="flex justify-between items-center mb-8">
          <div>
            <h4 className="text-2xl font-black text-[#06361f] tracking-tight">Website Messages</h4>
            <p className="text-[10px] font-black text-[#548064] uppercase tracking-widest mt-1">
              Received from homepage Get in Touch
            </p>
          </div>
          <span className="material-symbols-outlined text-[#006a28] text-3xl">mark_email_unread</span>
        </div>

        {dashboardData?.contactMessages?.length > 0 ? (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            {dashboardData.contactMessages.map((msg) => (
              <div key={msg.id} className="rounded-3xl border border-slate-100 bg-[#f8faf9] p-5">
                <div className="flex items-start justify-between gap-4 mb-3">
                  <div className="min-w-0">
                    <p className="font-black text-[#06361f] truncate">{msg.full_name}</p>
                    <p className="text-xs font-bold text-[#548064] truncate">{msg.email}</p>
                    {msg.phone && (
                      <p className="text-xs font-bold text-[#006a28] mt-0.5">{msg.phone}</p>
                    )}
                  </div>
                ...
                <p className="text-sm font-medium leading-relaxed text-[#39644a] line-clamp-3">
                  {msg.message}
                </p>
                <div className="mt-4 flex items-center justify-between gap-3">
                  <p className="text-[10px] font-black uppercase tracking-widest text-[#548064]">
                  ...
                  {msg.status === 'Unread' && (
                    <button
                      ...
                      className="shrink-0 rounded-xl bg-[#006a28] px-3 py-2 text-[9px] font-black uppercase tracking-widest text-white hover:bg-[#005a22] transition-colors"
                    >
                      Mark Read
                    </button>
                  )}
          ...
          <div className="rounded-3xl border border-dashed border-slate-200 p-10 text-center">
            <span className="material-symbols-outlined text-4xl text-[#548064]/50">inbox</span>
            <p className="mt-2 text-sm font-bold text-[#548064]">No website messages yet.</p>
          </div>
        )}
      </div>
